...
def run_knn_cosine_similarity():
    ...
    node_colors = [y_train[i].item() for i in range(len(y_train))]
    nx.draw_networkx_nodes(G, pos, node_size=100, node_color=node_colors, cmap=plt.cm.Blues, alpha=0.9)
    ...
    plt.title("Graph Visualization of Grape Dataset (Selected Nodes Highlighted)", fontsize=16)
    plt.axis('off')
    plt.show()
